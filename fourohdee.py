...
urlRe = re.compile(r'http://ais.channel4.com/asset/[0-9]+?[0-9]+')
...
def intercept_forward(f1, f2, maxlen=0):
    left = maxlen or 1000000000
    buffer = StringIO()
    while left:
        data = f1.read(min(left, 2048))
    ...
    while True:
        ...
        data = data.replace('break>', 'bleak>')
        f2.write(data)


class FourOhDeeHTTPProxyHandler(HTTPProxyHandler):
    ...
    def forward_response_body(self, f1, f2, contentlength):
        ...
        match = urlRe.match(self.url)
        if match:
            #~ print "Cleaning:",self.url
            intercept_forward(f1, f2, contentlength)
        else:
...
if __name__ == '__main__':
    server = FourOhDeeHTTPProxyServer()
    server.run()
